...
timestampn[0] = curr_time.tv_nsec; //storing time in nsecs
    ...
    for (;frame_count<=1000;frame_count++)
        {       clock_gettime(CLOCK_REALTIME, &curr_time); 
           ...
           if(diff[frame_count]>= 35000000)  // to calculate positive jitter we are considering only ones who are greater than avg 
            {
            jitter[frame_count] = diff[frame_count] - 35000000; 
         ...
         fd_set fds;
            ...
            /* EAGAIN - continue select loop unless count done. */
                        ...
                        printf("frame_acquisition_time=%ld\n", diff[frame_count]);
        }
