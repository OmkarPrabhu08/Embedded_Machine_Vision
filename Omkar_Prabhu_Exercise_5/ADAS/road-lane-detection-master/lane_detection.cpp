...
int main(int argc,char *argv[])
{
    ...
    //    inRange(frame, Scalar(0, 30, 30), Scalar(30, 255, 255), y_frame);
      //  inRange(gray, 120, 255, w_frame);
        ...
        inRange(hls_img, Scalar(20, 40, 40), Scalar(30, 200, 255), y_frame);
        inRange(hls_img, Scalar(0, 180, 0), Scalar(180, 255, 255), w_frame);
//erode(y_frame, y_frame, Mat(), Point(-1, -1), 1, 1, 1);
dilate(y_frame, y_frame, Mat(), Point(-1, -1), 4, 1, 1);
erode(y_frame, y_frame, Mat(), Point(-1, -1), 1, 1, 1);
dilate(y_frame, y_frame, Mat(), Point(-1, -1), 4, 1, 1);
erode(y_frame, y_frame, Mat(), Point(-1, -1), 1, 1, 1);

dilate(w_frame, w_frame, Mat(), Point(-1, -1), 3, 1, 1);
erode(w_frame, w_frame, Mat(), Point(-1, -1), 2, 1, 1);
dilate(w_frame, w_frame, Mat(), Point(-1, -1), 1, 1, 1);


        Canny(y_frame,y_frame, 20, 60, 3);
        Canny(w_frame,w_frame, 20, 60, 3);
        inRange(hls_img, Scalar(0, 0, 0), Scalar(180, 98, 35), b_frame);
    //    inRange(hls_img, Scalar(0, 115, 0), Scalar(255, 255, 255), w_frame);
                bitwise_or(y_frame, w_frame, yw_frame);
                bitwise_and(yw_frame, gray, and_frame);
    ...
    //bitwise_or(y_frame, w_frame, yw_frame);
        ...
        Mat cropped;
         cropped = w_frame(Rect(w_frame.cols/4,3*w_frame.rows/5,w_frame.cols/2,w_frame.rows/5));
        ...
        vector<Vec4i> lines;
        HoughLinesP(cropped, lines, 1, CV_PI/180, 35, 20, 25);
